...
int myPlotterDelay = 5;   // non-block delay in MICROSECONDS
unsigned long myPlotterStart; 


int myActivateDelay = 20;   // non-block delay in milliseconds
unsigned long myActivateStart; 

int myStopDelay = 5;   // non-block delay in milliseconds
unsigned long myStopStart; 

bool  mySwitch = false; 

void setup() {
   myPlotterStart  = micros();   
...
void loop() {
    ...
    if ( (micros() - myPlotterStart ) >= myPlotterDelay) {       
       myPlotterStart = micros();      //  reset the delay time
